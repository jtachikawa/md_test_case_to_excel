...
def append_df(df: pd.DataFrame, current_item_dict: dict) -> pd.DataFrame:
    # ここまでの項目をデータフレームに追加
    # current_item_dictの値をDataFrameに変換し、concatのリストにする
    _df = pd.concat([df, pd.DataFrame([current_item_dict])], ignore_index=True)
...
def convert_md_to_df(input_path: str, config_md: dict) -> pd.DataFrame:
    ...
    for i, line in enumerate(input_file):
        ...
        if re.match(config_md["mark"]["steps"], line):
            ...
            current_item_dict["sub-sub-item"] = line.replace(config_md["mark"]["sub-sub-item"],"").lstrip()
